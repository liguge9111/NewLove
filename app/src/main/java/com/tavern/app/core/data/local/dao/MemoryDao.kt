...
interface MemoryDao {
    ...
    @Query("SELECT * FROM chat_summaries WHERE id = :id")
    suspend fun getLatestSummaryById(id: Long): ChatSummaryEntity?

    @Update
    suspend fun updateSummary(summary: ChatSummaryEntity)

    @Query("DELETE FROM chat_summaries WHERE id = :id")
