...
class ChatViewModel @Inject constructor(
    ...
    init {
        viewModelScope.launch {
            ...
            if (existingSession != null) {
                _currentModelConfigId.value = existingSession.modelConfigId
                val mode = runCatching {
                    InteractiveMode.valueOf(existingSession.interactiveMode)
                ...
                if (mode == InteractiveMode.CHAT) {
                    chatRepository.updateSessionMode(existingSession.id, InteractiveMode.TEXT_ADVENTURE)
                }
                _sessionId.value = existingSession.id

                // 会话从未有过角色消息（含功能上线前的旧会话）：补写开局到最前面
                val existingMessages = chatRepository.getMessages(existingSession.id)
     ...
     */
    private suspend fun generateReply(sessionId: Long, attempt: Int = 0, vnRetry: Boolean = false) {
        val plugins = pluginRepository.getEnabledPlugins(cardId)
        ...
        result.onSuccess { reply ->
            if (reply.isNotBlank()) {
                ...
                if (SafetyFilter.isRejection(reply)) {
                    ...
                    val processed = PluginEngine.applyReply(plugins, reply)
